            ...
            <form method="GET" action="{{ route('admin.peminjaman.create') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="kategori" class="w-full border border-gray-300 p-2 rounded-lg">
                    ...
                    <div>
                        ...
                        <select name="kondisi" class="w-full border border-gray-300 p-2 rounded-lg">
                ...
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <div class="flex flex-wrap items-center gap-4">
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="stok_tersedia" value="1"
        ...
        <form method="POST" action="{{ route('admin.peminjaman.store') }}" id="form-peminjaman">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_pinjam" value="{{ old('tanggal_pinjam', date('Y-m-d')) }}"
                        min="{{ date('Y-m-d') }}" class="w-full border border-gray-300 p-2 rounded-lg" required>
                </div>
            ...
            @else
                            ...
                            @foreach ($alat as $a)
                                <tr class="hover:bg-gray-50 transition" data-stok="{{ $a->stok }}">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <input type="checkbox" name="alat[{{ $loop->index }}][id_alat]"
                                            value="{{ $a->id_alat }}" id="alat_{{ $a->id_alat }}"
                                                        ...
                                                        <p class="text-xs text-red-600 mt-1 font-medium">
                                                            <svg class="w-4 h-4 inline mr-1" fill="none"
                                                                ...
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    stroke-width="2"
                                                                    d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.998-.833-2.732 0L4.284 16.5c-.77.833.192 2.5 1.732 2.5z" />
                                                            </svg>
                                                            Stok habis
                                                        </p>
                                                    @endif
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <span
                                                class="text-lg font-bold {{ $a->stok > 0 ? 'text-green-600' : 'text-red-600' }}">
                                                {{ $a->stok }}
                                            </span>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            <input type="number" name="alat[{{ $loop->index }}][jumlah]"
                                                id="jumlah_{{ $a->id_alat }}" min="1"
                                                max="{{ $a->stok }}"
                                                value="{{ old('alat.' . $loop->index . '.jumlah', 1) }}"
                                                class="w-full border border-gray-300 p-2 rounded-lg text-center jumlah-input"
                                                placeholder="Jumlah" disabled data-max="{{ $a->stok }}"
                ...
                <div class="mt-4 space-y-4">
                    <div
                        class="flex flex-wrap items-center justify-between gap-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const filterSelects = document.querySelectorAll(
                    'select[name="kategori"], select[name="kondisi"], select[name="status"]');
