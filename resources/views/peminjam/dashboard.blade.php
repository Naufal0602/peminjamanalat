        ...
        <div class="bg-white rounded-xl shadow p-6 mb-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <a href="{{ route('peminjam.peminjaman.create') }}"
                    class="bg-blue-50 hover:bg-blue-100 rounded-lg p-4 text-center transition-colors duration-200">
                ...
                <a href="{{ route('peminjam.peminjaman.index') }}"
                    class="bg-green-50 hover:bg-green-100 rounded-lg p-4 text-center transition-colors duration-200">
                ...
                <a href="{{ route('peminjam.pengembalian.index') }}"
                    class="bg-purple-50 hover:bg-purple-100 rounded-lg p-4 text-center transition-colors duration-200">
                ...
                <a href="{{ route('profile.edit') }}"
                    class="bg-yellow-50 hover:bg-yellow-100 rounded-lg p-4 text-center transition-colors duration-200">
                            ...
                            <div
                                        ...
                                        <h3 class="font-medium text-gray-800">
                                            ...
                                            @if ($peminjaman->detailPeminjaman->count() > 1)
                                                <span class="text-sm text-gray-500">+
                                                    {{ $peminjaman->detailPeminjaman->count() - 1 }} alat lainnya</span>
                                            @endif
                                        </h3>
                                ...
                                <div class="grid grid-cols-2 gap-4 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <p class="font-medium">
                                            {{ $peminjaman->tanggal_kembali_rencana ? \Carbon\Carbon::parse($peminjaman->tanggal_kembali_rencana)->format('d M Y') : 'N/A' }}
                                        </p>
                                    </div>
                                </div>

                                @php
                                    $daysLeft = $peminjaman->tanggal_kembali_rencana
                                        ? floor(
                                            \Carbon\Carbon::parse($peminjaman->tanggal_kembali_rencana)->diffInDays(
                                                now(),
                                                false,
                                            ),
                                        )
                                ...
                                <div class="mt-4">
                                    <div class="flex justify-between text-sm mb-1">
                                        ...
                                        <span
                                            class="font-medium @if ($daysLeft < 3) text-red-600 @else text-gray-700 @endif">
                                            @if ($daysLeft > 0)
                                                {{ $daysLeft }} hari lagi
                                            @elseif($daysLeft == 0)
                                                Hari ini
                                    ...
                                    <div class="w-full bg-gray-200 rounded-full h-2">
                                        @php
                                            $totalDays = max(
                                                1,
                                                optional($peminjaman->tanggal_pinjam)
                                                    ? \Carbon\Carbon::parse($peminjaman->tanggal_pinjam)->diffInDays(
                                            ...
                                            $percentage = min(
                                                100,
                                                max(0, (($totalDays - max(0, $daysLeft)) / $totalDays) * 100),
                                            );
                                        @endphp
